...
import { drawGameBackground } from '@shared/utils/canvasHelpers'
import { teamLogoUrl } from '@shared/utils/bsdApi'
...
export default memo(function NextGame({ game, onCta, loading, ridesCount, homePosition, awayPosition }) {
  ...
  if (ridesCount != null) {
    dynamicPills.push({ icon: 'rides', text: `${ridesCount} ${ridesCount === 1 ? 'viagem' : 'viagens'}` })
  } else if (game.pills) {
    ...
    <section className={styles.card} aria-labelledby="game-title">
      ...
      <div className={styles.content}>
        ...
        <button
          ...
          onClick={() => onCta?.(game)}
          aria-label={`Ver viagens para ${game.homeTeam.name} x ${game.awayTeam.name}`}
        >
